...
type TournamentTab = "leaderboard" | "picks" | "all-picks" | "admin";
...
function formatCountdown(ms: number | null): { label: string; urgent: boolean; past: boolean } | null {
  ...
  if (ms <= 0) return { label: "Locked", urgent: true, past: true };
  const s = Math.floor(ms / 1000);
  const d = Math.floor(s / 86400);
  const h = Math.floor((s % 86400) / 3600);
  const m = Math.floor((s % 3600) / 60);
  if (d > 0) return { label: `${d}d ${h}h`, urgent: false, past: false };
  if (h > 0) return { label: `${h}h ${m}m`, urgent: true, past: false };
  return { label: `${m}m`, urgent: true, past: false };
}

function CountdownBadge({ deadline, theme }: { deadline: string; theme: TournamentTheme }) {
...
function TournamentCard({
  ...
  const isPast = new Date(tournament.deadline) <= new Date();

  return (
    <button
      onClick={onClick}
      className="w-full text-left theme-transition"
      style={{
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
        <div>
          <div style={{ fontSize: 13, color: theme.accent, fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 4 }}>
        ...
        <CountdownBadge deadline={tournament.deadline} theme={theme} />
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ fontSize: 12, color: theme.dim }}>
          {new Date(tournament.deadline).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}
        </div>
        <div
          style={{
            fontSize: 11,
...
export function Dashboard({ user, onLogout }: { user: User; onLogout: () => void }) {
      ...
      <div style={{ background: DEFAULT_THEME.bg, minHeight: "100vh" }} className="theme-transition">
        ...
        <header style={{ background: "linear-gradient(180deg, #040f08 0%, #09170e 100%)", borderBottom: `1px solid ${DEFAULT_THEME.border}` }}>
          <div style={{ maxWidth: 600, margin: "0 auto", padding: "16px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <h1 style={{ fontSize: 20, fontWeight: 800, color: DEFAULT_THEME.accent, margin: 0, letterSpacing: 0.5 }}>
                Golf Majors 2026
              </h1>
              <p style={{ fontSize: 12, color: DEFAULT_THEME.muted, margin: "2px 0 0" }}>
                {user.name}
              </p>
            </div>
  ...
  return (
    <div style={{ background: activeTournament ? theme.bg : DEFAULT_THEME.bg, minHeight: "100vh" }} className="theme-transition">
      {/* Header */}
        ...
        <div style={{ maxWidth: 600, margin: "0 auto", padding: "12px 20px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
            <button
              ...
              style={{ background: "none", border: "none", color: theme.accent, cursor: "pointer", fontSize: 14, fontWeight: 600, padding: 0 }}
            >
              ← Back
            </button>
            {activeTournament && (
              <h1 style={{ fontSize: 16, fontWeight: 700, color: theme.accent, margin: 0 }}>
                {activeTournament.name}
              </h1>
            )}
            {!activeTournament && isAdmin && (
              <h1 style={{ fontSize: 16, fontWeight: 700, color: theme.accent, margin: 0 }}>Admin</h1>
            )}
          </div>
